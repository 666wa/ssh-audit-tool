...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description='SSH命令核查工具 - 验证SSH操作是否已报备',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
示例用法:
  python -m ssh_audit_tool.main -o 操作命令表.xlsx -r 报备表.xlsx
  python -m ssh_audit_tool.main -o 操作命令表.xlsx -r 报备表.xlsx --output 结果.xlsx
  python -m ssh_audit_tool.main -o 操作命令表.xlsx -r 报备表.xlsx --no-timestamp
        """
    )
    ...
    parser.add_argument(
        '-o', '--operation',
        required=True,
        help='操作命令表文件路径（Excel格式）'
    )
    
    parser.add_argument(
        '-r', '--report',
        required=True,
        help='报备表文件路径（Excel格式）'
    )
...
def main():
    ...
    try:
        logger.info("=" * 60)
        logger.info("SSH命令核查工具启动")
        logger.info("=" * 60)
        
        # 1. 读取操作命令表
        logger.info(f"操作命令表: {args.operation}")
        operation_df = FileHandler.read_operation_log(args.operation)
        
        # 1.5 从文件名提取日期
        file_date = DateExtractor.extract_date_from_filename(args.operation)
        if file_date:
        ...
        report_df = FileHandler.read_report_table(args.report)
        ...
        for idx, row in operation_df.iterrows():
            operation_content = str(row.get('操作内容', ''))
            resource_ip = str(row.get('资源IP', ''))
            resource_name = str(row.get('资源名称', ''))
            main_account = str(row.get('主账号名称', ''))
            operation_time = str(row.get('操作时间', ''))  # 获取操作时间
            ...
            # 优先使用从账号名称（如果存在）
            sub_account = str(row.get('从账号名称', ''))
            default_account = sub_account if sub_account and sub_account != 'nan' else main_account
            
            # 核查单条记录（不再传入操作时间，使用文件日期）
            result, matched_row, detail = auditor.audit_record(operation_content, source_ip, default_account)
